...
export default function Community() {
  // Load shared adventures from localStorage
  const [adventures, setAdventures] = useState([]);
  useEffect(() => {
    const saved = localStorage.getItem('sharedAdventures');
    if (saved) setAdventures(JSON.parse(saved));
  }, []);
  ...
  {displayResults.length > 0 ? (
      ...
      {/* Shared Adventures Section - Now at the bottom with clear separation */}
      {adventures.length > 0 && (
        <div className="mt-20 pt-20 border-t-4 border-white/10">
          <h3 className="text-3xl font-bold text-white mb-8 text-center bg-gradient-to-r from-purple-400 via-pink-400 to-tripmindA bg-clip-text text-transparent">
            Your Travel Stories
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {adventures.map((adv, idx) => (
              <div key={idx} className="rounded-3xl overflow-hidden shadow-xl bg-white/10 backdrop-blur-lg border border-white/20 transform transition-all duration-300 hover:scale-105 hover:shadow-2xl">
                {adv.photo && (
                ...
                <div className="p-6">
                  ...
                  <p className="text-white/90 mb-2 line-clamp-3">{adv.story}</p>
